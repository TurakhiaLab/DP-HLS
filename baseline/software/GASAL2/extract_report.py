...
# Directory containing the nsys reports
PROFILING_DIR = "profiling_reports"

# List of report names to process
REPORT_NAMES = [
    "report_global_1000k",
    "report_local_1000k",
    "report_banded_local_1000k",
]

# those are the thoroughput numbers taken from the paper. 
DP_HLS_THROUGHPUT = {
    "report_global_1000k": 2.85e6,
    "report_local_1000k": 2.71e6,
    "report_banded_local_1000k": 4.77e6,
}
...
def extract_nsys_report(report_name):
    ...
    """
    input_file = os.path.join(PROFILING_DIR, f"{report_name}.nsys-rep")
    output_base = os.path.join(PROFILING_DIR, f"exported_{report_name}")
...
def find_alignment_kernel_time(json_file):
    ...
    kernel_pattern = re.compile(r'gasal_\w+_kernel<')
...
def clean_existing_exports():
    ...
    for report_name in REPORT_NAMES:
        json_file = os.path.join(PROFILING_DIR, f"exported_{report_name}_gpukernsum.json")
...
def main():
    ...
    print("=" * 80)
    print(f"{'Report Name':<30} {'Time (s)':<15} {'DP-HLS (align/s)':<20} {'GPU (align/s)':<20} {'Raw Speedup':<15}")
    print("-" * 100)
    
    for report_name, info in results.items():
        dp_hls_throughput = DP_HLS_THROUGHPUT[report_name]
        gpu_throughput = 1000 * 1000 / info['total_time_sec']
        raw_speedup = dp_hls_throughput / gpu_throughput
        print(f"{report_name:<30} {info['total_time_sec']:<15.6f} {dp_hls_throughput:<20.2f} {gpu_throughput:<20.2f} {raw_speedup:<15.2f}")
    
    # Print ISO-cost summary
    print("\n" + "=" * 80)
    print("SUMMARY: ISO-Cost Throughput and Speedup")
    print("=" * 80)
    print(f"{'Report Name':<30} {'DP-HLS/$ (align/$)':<25} {'GPU/$ (align/$)':<25} {'ISO-Cost Speedup':<20}")
    print("-" * 100)
    ...
    for report_name, info in results.items():
        ...
        print(f"{report_name:<30} {dp_hls_per_dollar:<25.2f} {gpu_per_dollar:<25.2f} {iso_cost_speedup:<20.2f}")
    
    # Save results to a summary JSON file
    summary_file = os.path.join(PROFILING_DIR, "alignment_kernel_summary.json")
